...
/*
    setInterval(function(){
        if ($('#comite_DoctorsNote_detailview_tabs').length && !$('#comite_DoctorsNote_detailview_tabs').hasClass('processed')) {
            $('#comite_DoctorsNote_detailview_tabs').addClass('processed');
            var $id = $('#content .moduleTitle ~ .actionsContainer #form input[name=record]').val();
            $('form input[name=return_module][value=comite_DoctorsNote]').each(function(){
                $('~input[name=return_id]', this).val($id);
            });
        }
    }, 1000);
// this doesn't work because the embedded form ends up using "return_id" for "relate_to" which is incorrect. it should be using parent_id or something
*/
</script>
{/literal}
