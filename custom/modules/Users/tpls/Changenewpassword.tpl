...
<script type='text/javascript'><!--
    function validateSecurityQuestion() {
        if(document.getElementById('security_question_answer').value.length) {
            return true;
        }
...
<td align="center">
		...
		{/if}
			...
			{$USERNAME_FIELD}
                ...
                <tr>
                        <td scope="row" colspan="2">{$MOD.LBL_CHANGE_SYSTEM_PASSWORD}</td>
                </tr>
		<tr>
		...
		<tr>
			...
			<td width="30%"><input type="password" size="26" tabindex="2" id="confirm_pwd" name="confirm_pwd" value="" onkeyup="password_confirmation();" /> <div id="comfirm_pwd_match" class="error" style="display: none;">mis-match</div></td>
                ...
                <tr>
			<td scope="row" colspan="2"><h3>{$MOD.SECURITY_QUESTION_TITLE}:</h3></td>
                </tr>
                <tr>
			<td scope="row" colspan="2">{$SECURITY_QUESTION}:</td>
                </tr>
                <tr>
			<td colspan="2"><input type="text" size="40" tabindex="2" autocomplete="off" id="security_question_answer" name="security_question_answer" value="" /></td>
                </tr>
		<tr>
			<td>{$CAPTCHA}</td>
		</tr>
