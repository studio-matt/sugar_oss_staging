...
if (isset($_REQUEST['guid'])) {
    ...
    if (!empty($row)) {
        ...
        if (!$expired) {
            ...
            $usr = new user();
            $usr_id = $usr->retrieve_user_id($row['username']);
            $usr->retrieve($usr_id);
            if (isset($_REQUEST['login']) && $_REQUEST['login'] == '1') {
                if(!$usr->check_password_rules($_POST['new_password'])) {
                    $redirect = 0;
                    $error_string = $mod_strings['LBL_REQUIRE_SECURE_PASSWORD'];
                
                } elseif(strtolower($usr->security_question_answer_c) != strtolower($_POST['security_question_answer'])) {
                    $redirect = 0;
                    $error_string = $mod_strings['LBL_SECURITY_QUESTION_ANSWER_INCORRECT'];
                
                } elseif ($row['username'] == $_POST['user_name']) {

                    $user_hash = strtolower(md5($_POST['new_password']));
                    $usr->setPreference('loginexpiration', '0');
                    //set new password
                    $now = TimeDate::getInstance()->nowDb();
                    $usr->user_hash=$user_hash;
                    $usr->system_generated_password=0;
                    $usr->pwd_last_changed=$now;
                    $usr->save();
...
$sugar_smarty->assign('SECURITY_QUESTION', $app_list_strings['security_question_list'][$usr->security_question_c]);
if(isset($error_string)) {
    $sugar_smarty->assign('EXPIRATION_TYPE', $error_string);
}
if(isset($_GET['error_password'])) {
    $sugar_smarty->assign('EXPIRATION_TYPE', $_GET['error_password']);
}
...
$sugar_smarty->assign('SUBMIT_BUTTON', '<input title="' . $mod_strings['LBL_LOGIN_BUTTON_TITLE']
        . '" class="button" '
        . 'onclick="if(!set_password(form,newrules(' . $rules . '))) return false; if(!validateSecurityQuestion()) return false; validateCaptchaAndSubmit();" '
...
$sugar_smarty->display('custom/modules/Users/tpls/Changenewpassword.tpl');
$view->displayFooter();
?>
