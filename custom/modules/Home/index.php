...
$hide = array('CalendarDashlet', 'MyTasksDashlet', 'MyMeetingsDashlet');
$hideIds = array();
if (SugarThemeRegistry::current()->name == 'ComiteMDPortal') {
    // Remove the calendar from the patient homepage
    foreach($dashlets as $id => $dashlet) {
        if (in_array($dashlet['className'], $hide)) {
            $hideIds[] = $id;
        }
    }
}
if ($hideIds) {
    $pages[0]['columns'][0]['dashlets'] = array_diff($pages[0]['columns'][0]['dashlets'], $hideIds);
}
...
/**
 * Start Custom Code for HLA Button
 * @author Stephen Ostrow <stephen@whoisstudio.com>
 * @since 2013-01-28
 */
$user = $current_user;
$user->get_linked_beans('contacts_users_1', 'Contact');
if(count($user->contacts_users_1->beans)) {
    $contact = $user->contacts_users_1->beans[reset(array_keys($user->contacts_users_1->beans))];
    $hla_started = $contact->hla_survey_step_c;
}
if(!isset($hla_started) || !strlen($hla_started)) {
    $sugar_smarty->assign('comiteHlaButton', 'Begin Here.');
} elseif($hla_started == 'hla_complete') {
    $sugar_smarty->assign('comiteHlaButton', 'Finished.');
} else {
    $sugar_smarty->assign('comiteHlaButton', 'Continue Here.');
}
/**
 * End Custom Code for HLA Button
 */
