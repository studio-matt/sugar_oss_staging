...
function get_module_name(/*child_field*/)
{
	...
	if(typeof(window.document.forms['DetailView']) == 'undefined') {
		...
		//you allow normal processing to continue.  For an example, see trackdetailview.html/php in campaigns module
	        detailViewForm = typeof(window.document.forms['DetailView'].elements) != 'undefined' ?  window.document.forms['DetailView'] : window.document.forms['DetailView'][index];
...
function showSubPanel(child_field,url,force_load,layout_def_key)
{
	...
	if (force_load || typeof( child_field_loaded[child_field] ) == 'undefined')
	{
		...
		{
			var module = get_module_name(child_field);
			var id = get_record_id(child_field);
            if ( typeof(layout_def_key) == 'undefined' || layout_def_key == null ) {
                layout_def_key = get_layout_def_key(child_field);
            }
          //  console.log(module);
                    if(module=='comite_DoctorsNote'){
            //            console.log('hi');
                        //document.location.href='index.php?module=comite_DoctorsNote&action=DetailView&record='+id;
