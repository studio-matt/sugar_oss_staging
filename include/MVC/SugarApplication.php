			...
			if(isset($_SESSION['hasExpiredPassword']) && $_SESSION['hasExpiredPassword'] == '1'){
				if( $this->controller->action!= 'Save' && $this->controller->action != 'Logout') {
					...
					$this->handleOfflineClient();
                         ...
                         * Adding code to redirect if user needs to update security question
                         */
                        } elseif (isset($_SESSION['hasExpiredSecurityQuestion']) && $_SESSION['hasExpiredSecurityQuestion'] == '1') {
                            if ($this->controller->action != 'Save' && $this->controller->action != 'Logout') {
                                $this->controller->module = 'comite_SecurityQuestion';
                                $this->controller->action = 'Change';
                                $record = $GLOBALS['current_user']->id;
                            } else {
                                $this->handleOfflineClient();
                            }
