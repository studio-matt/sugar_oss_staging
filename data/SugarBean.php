...
//    	require_once('data/Link.php');
        ...
        if (!empty($fieldDefs))
        {
            foreach ($fieldDefs as $name=>$properties)
            {
                if (array_search('relate',$properties) === 'type' || array_search('relateBoneStudy',$properties) === 'type')
                {
...
*/
        ...
        {
            if (($def [ 'type' ] == 'relate'||$def [ 'type' ] == 'relateBoneStudy') && isset ( $def [ 'id_name'] ) && isset ( $def [ 'link'] ) && isset ( $def[ 'save' ]) )
            {
		...
		foreach($this->field_defs as $fieldDef)
		{
			...
			if(!isset($this->processed_dates_times[$field]))
			{
				...
				elseif(isset($this->field_name_map[$field]['type']))
				{
					...
					if(($type == 'relate'||$type == 'relateBoneStudy')  && isset($this->field_name_map[$field]['custom_module']))
					{
						$type = $this->field_name_map[$field]['type'];
					}
    ...
    */
    function retrieve($id = -1, $encode=true,$deleted=true)
    {
        ...
        foreach ( $this->field_defs as $key => $def )
        {
            if (($def [ 'type' ] == 'relate'||$def [ 'type' ] == 'relate') && isset ( $def [ 'id_name'] ) && isset ( $def [ 'link'] ) && isset ( $def[ 'save' ])) {
                if (isset($this->$key)) {
    ...
    */
	...
	function create_new_list_query($order_by, $where,$filter=array(),$params=array(), $show_deleted = 0,$join_type='', $return_array = false,$parentbean=null, $singleSelect = false, $ifListForExport = false)
    {
        ...
        foreach($fields as $field=>$value)
        {
            ...
            if(($data['type'] == 'relate'||$data['type'] == 'relateBoneStudy') && isset($data['link']))
            {
        ...
        $class = get_class($this);
            ...
            while ($max_per_page == -99 || ($index < $row_offset + $max_per_page))
            {
                ...
                if (property_exists($temp, 'currency_id') && -99 == $temp->currency_id)
                {
                    ...
                    {
                        if (isset($temp_field['type']) && ('relate' == $temp_field['type']||'relateBoneStudy' == $temp_field['type'])
...
/*
    ...
    */
    function fill_in_relationship_fields(){
        ...
        {
            if((0 == strcmp($field['type'],'relate')||0 == strcmp($field['type'],'relateBoneStudy')) && !empty($field['module']))
            {
                $name = $field['name'];
                if(empty($this->$name))
                {
